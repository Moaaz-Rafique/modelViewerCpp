...
void main()
{
	Color =  texture(texture1, TexCoords.st) * vec4(Point_light(Normal, 1) , 1.0f);
}
...
vec3 Point_light(vec3 normal, int index)
{
	...
	vec3 specular = pow(max(dot(N, Half), 0.0), 64.0f) * vec3(0.0f, 0.0f, 1.0f) * attenuation * light.specularIntensity * texture(spec_map, TexCoords.st).rgb;
